...
open class ItemUpdateAPI {
    ...
    #if canImport(Combine)
    @available(OSX 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
    open class func updateItem(itemId: String, updateItemRequest: UpdateItemRequest, apiResponseQueue: DispatchQueue = JellyfinAPI.apiResponseQueue) -> AnyPublisher<Void, Error> {
        return Future<Void, Error>.init { promise in
            updateItemWithRequestBuilder(itemId: itemId, updateItemRequest: updateItemRequest).execute(apiResponseQueue) { result -> Void in
                switch result {
                case .success:
                    promise(.success(()))
                case let .failure(error):
    ...
    open class func updateItemWithRequestBuilder(itemId: String, updateItemRequest: UpdateItemRequest) -> RequestBuilder<Void> {
        ...
        urlPath = urlPath.replacingOccurrences(of: "{itemId}", with: itemIdPostEscape, options: .literal, range: nil)
        let URLString = JellyfinAPI.basePath + urlPath
        let parameters = JSONEncodingHelper.encodingParameters(forEncodableObject: updateItemRequest)
