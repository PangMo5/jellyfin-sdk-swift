...
open class EnvironmentAPI {
    ...
    #if canImport(Combine)
    @available(OSX 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
    open class func validatePath(validatePathRequest: ValidatePathRequest, apiResponseQueue: DispatchQueue = JellyfinAPI.apiResponseQueue) -> AnyPublisher<Void, Error> {
        return Future<Void, Error>.init { promise in
            validatePathWithRequestBuilder(validatePathRequest: validatePathRequest).execute(apiResponseQueue) { result -> Void in
                switch result {
                case .success:
                    promise(.success(()))
                case let .failure(error):
    ...
    open class func validatePathWithRequestBuilder(validatePathRequest: ValidatePathRequest) -> RequestBuilder<Void> {
        ...
        let URLString = JellyfinAPI.basePath + urlPath
        let parameters = JSONEncodingHelper.encodingParameters(forEncodableObject: validatePathRequest)
